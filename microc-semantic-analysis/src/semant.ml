...
let check_func f scope loc =
  ...
  with Symbol_table.DuplicateEntry ->
    Util.raise_semantic_error loc @@    "function "^ f.fname ^ " already defined"

let check_type loc t = 
  match t with
  | TypA(TypA(_,_),_) -> Util.raise_semantic_error loc "Cannot define multidimensional array"
  | TypA(TypV,_) -> Util.raise_semantic_error loc "Trying to define a void array"
  | TypA(t,Some i) when i < 1 -> Util.raise_semantic_error loc  "Array must have size > 0"
  | TypA(t, None) -> Util.raise_semantic_error loc "Array must be declared with an initial size"
...
let check_var_decl (t,i) scope loc =
  let check_var_type loc t = 
    match t with
    | TypV -> Util.raise_semantic_error loc "Trying to define a void variable"
    | _ -> check_type loc t
  ...
  try 
    Symbol_table.add_entry i (loc, t) scope.var_symbols |> ignore
  with DuplicateEntry ->
    Util.raise_semantic_error loc @@ "Variable" ^ i ^ " already defined in current scope"
...
let check_topdecl scope node = 
  ...
  |Vardec(t,i) -> check_var_decl (t,i) scope node.loc
